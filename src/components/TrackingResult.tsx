  ...
  return (
    <div>
      <div className="flex min-h-screen flex-col bg-gray-100">
        <header className="bg-[#303030] py-4 shadow-lg">
          <div className="container mx-auto flex items-center justify-between px-4">
            <img src={logo} alt="Logo" className="h-12 sm:h-16 object-contain" />
            <h1 className="text-2xl sm:text-3xl font-bold text-white">Seguimiento de Guía</h1>
          </div>
        </header>
          ...
          <Card className="shadow-xl mb-6 bg-white rounded-lg">
            <CardHeader className="border-b-2 border-[#303030]">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <CardTitle className="text-xl sm:text-2xl font-semibold text-gray-900">Guía #{guia}</CardTitle>
                ...
                <div className="flex space-x-6 items-center justify-between w-full z-10">
                  {timelineSteps.map((step, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`relative flex flex-col items-center ${isCompleted || isCurrent ? "text-[#303030]" : "text-gray-400"}`}
                      >
                        <div
                          className={`flex items-center justify-center ${isCurrent ? "p-1 rounded-full ring-2 ring-[#303030] ring-offset-2 ring-offset-white transition-all duration-300" : ""}`}
                        >
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center border-4 transition-colors duration-500 ${isCompleted || isCurrent ? "border-[#303030]" : "border-gray-300"}`}
                          >
                            {step.icon}
                          </div>
                        </div>
